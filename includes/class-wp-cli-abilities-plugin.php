...
class WP_CLI_Abilities_Plugin {
	...
	public function register_settings(): void {
		...
		add_settings_section(
			...
			function () {
				echo '<p>' . esc_html__(
					'Safety controls for ability execution. Dangerous commands (db, config, shell) are always blocked.',
					'wp-cli-abilities'
				) . '</p>';
			},
			'wp-cli-abilities'
		...
		add_settings_field(
			'wp_cli_abilities_allow_eval',
			__( 'Allow eval commands', 'wp-cli-abilities' ),
			function () {
				$checked = get_option( 'wp_cli_abilities_allow_eval', false );
				printf(
					'<label><input type="checkbox" name="wp_cli_abilities_allow_eval" value="1" %s />
					%s</label>
					<p class="description">%s</p>',
					checked( $checked, true, false ),
					esc_html__( 'Enable wp eval and wp eval-file commands', 'wp-cli-abilities' ),
					esc_html__( 'Allows arbitrary PHP code execution. Only enable if you trust all API consumers.', 'wp-cli-abilities' )
				);
			},
			'wp-cli-abilities',
			'wp_cli_abilities_guardrails'
